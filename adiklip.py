...
def fit_residual_fake_planets(fake_planets, psf_model_img, max_fit_rad, planet_coadd_img, log_fobj=sys.stdout):
    psf_width = psf_model_img.shape[0]
    psf_cent_xy = ((psf_width - 1)/2., (psf_width - 1)/2.)

    psf_model_peak = np.max(psf_model_img)
    fit_fluxes = list()

    data_cent_xy = ((planet_coadd_img.shape[1] - 1)/2., (planet_coadd_img.shape[0] - 1)/2.) 
    Ydata, Xdata = np.indices(planet_coadd_img.shape)

    for R, PA, flux in fake_planets:
        log_fobj.write("Fitting fake planet PSF with flux %.2e inserted at R=%0.2f, PA=%0.2f\n" % (flux, R, PA))
        src_img = planet_coadd_img.copy()
        bgguess = 0.
        ampguess = flux
        pos_deltaxy = ( R*np.cos(np.deg2rad(PA + 90)), R*np.sin(np.deg2rad(PA + 90)) )
        pos_xy = ( data_cent_xy[0] + R*np.cos(np.deg2rad(PA + 90)),\
                   data_cent_xy[1] + R*np.sin(np.deg2rad(PA + 90)) )

        p0 = np.array([bgguess, ampguess, pos_deltaxy[0], pos_deltaxy[1]])
        p_min = [bgguess - 2*ampguess, -2*ampguess, pos_deltaxy[0], pos_deltaxy[1]]
        p_max = [bgguess + 2*ampguess, 2*ampguess, pos_deltaxy[0], pos_deltaxy[1]]
        p_bounds = [(p_min[i], p_max[i]) for i in range(len(p_min))]

        Rsrc_sqrd = (Xdata - pos_xy[0])**2 + (Ydata - pos_xy[1])**2
        exc_ind = np.where(Rsrc_sqrd > max_fit_rad**2)
        src_img[exc_ind] = np.nan
        fit_ind = np.isfinite(src_img)
        exc_ind = np.isnan(src_img)

        guess_cost = eval_simple_psf_model(p = p0, src_img = src_img, psf_img = psf_model_img, fit_ind = fit_ind, exc_ind = exc_ind)
        log_fobj.write("Cost of guess model = %e\n" % guess_cost)
        start_time = time.time()                  
        p_sol, final_cost, info = fmin_l_bfgs_b(func = eval_simple_psf_model, x0 = p0,
                                                args = (src_img, psf_model_img, fit_ind, exc_ind),
                                                approx_grad = True, bounds = p_bounds, epsilon=1e-5, factr=1e4, maxfun=1e6, disp=0)
        end_time = time.time()
        exec_time = end_time - start_time
        delimiter = ', '
        log_fobj.write( "Solution parameters: %s\n" % delimiter.join(["%.3e" % p_sol[i] for i in range(p_sol.shape[0])]) )
        log_fobj.write("Took %dm%02ds to optimize the source model, to final cost %e\n" % (int(exec_time/60.), exec_time - 60*int(exec_time/60.), final_cost))
        fit_fluxes.append(p_sol[1])
        log_fobj.write("Best-fit flux: %.3e\n" % fit_fluxes[-1] )
    return fit_fluxes

def get_residual_fake_planet_stats(config_dict, Phi_0, R_p, PA_p, flux_p, flux_p_fit, fwhm, planet_coadd_img, planet_med_img,
    ...
    sys.stdout.write("RMS counts in KLIP results:\n")
    for rad_ind in op_rad:
        R2 = R_out[rad_ind]
        if rad_ind == 0:
            R1 = R_inner
        ...
        sys.stdout.write("\tannulus %d/%d: %.2f in KLIP sub'd, derotated, coadded annlus\n" % (rad_ind+1, len(op_rad), coadd_annular_rms[-1]))
        sys.stdout.write("\tannulus %d/%d: %.2f in KLIP sub'd, derotated, median annlus\n" % (rad_ind+1, len(op_rad), med_annular_rms[-1]))
    for i, rad_ind in enumerate(op_rad):
        ...
        if R1 < R_p and R_p < R2:
            ...
            sys.stdout.write("Measuring fake planet signal at location %d, %d in residual image\n" % (round_xy_p[0], round_xy_p[1]))
            ...
            psffit_planet_snr = flux_p_fit / coadd_annular_rms[i]
            psffit_planet_thrupt = flux_p_fit / flux_p
            psffit_min_detect = 5 * coadd_annular_rms[i] / psffit_planet_thrupt
            
            log_fobj.write("Peak planet pixel value in final co-added planet image: %.2e => SNR = %0.2f, throughput = %0.2f, min. detect. input flux = %.2e\n" % \
                           (coadd_planet_peak, coadd_planet_snr, coadd_planet_thrupt, coadd_min_detect))
            log_fobj.write("Peak planet pixel value in median of de-rotated planet images: %.2e => SNR = %0.2f, throughput = %0.2f, min. detect. input flux = %.2e\n" % \
                           (med_planet_peak, med_planet_snr, med_planet_thrupt, med_min_detect))
            log_fobj.write("PSF fit throughput = %0.2f => SNR = %0.2f, min. detect. input flux = %.2e\n" % (psffit_planet_thrupt, psffit_planet_snr, psffit_min_detect))
            sys.stdout.write("Peak planet pixel value in final co-added planet image: %.2e => SNR = %0.2f, throughput = %0.2f, min. detect. input flux = %.2e\n" % \
                           (coadd_planet_peak, coadd_planet_snr, coadd_planet_thrupt, coadd_min_detect))
            sys.stdout.write("Peak planet pixel value in median of de-rotated planet images: %.2e => SNR = %0.2f, throughput = %0.2f, min. detect. input flux = %.2e\n" % \
                           (med_planet_peak, med_planet_snr, med_planet_thrupt, med_min_detect))
            sys.stdout.write("PSF fit throughput = %0.2f => SNR = %0.2f, min. detect. input flux = %.2e\n" % (psffit_planet_thrupt, psffit_planet_snr, psffit_min_detect))
    return coadd_annular_rms, coadd_min_detect, med_annular_rms, med_min_detect, psffit_min_detect
...
def eval_simple_psf_model(p, src_img, psf_img, fit_ind, exc_ind, res_img_fname=None, model_img_fname=None):
    ...
    pos_xy = (src_cent_xy[0] + p[2], src_cent_xy[1] + p[3])
    model_img = superpose_srcmodel(data_img = np.zeros(src_img.shape) + bg, srcmodel_img = amp*psf_img,
                                   srcmodel_destxy = pos_xy, srcmodel_centxy = psf_cent_xy)
    res_img = np.zeros(src_img.shape)
    res_img[fit_ind] = src_img[fit_ind] - model_img[fit_ind]
...
def get_ref_and_pix_tables(xycent, fr_shape, N_fr, op_fr, mode_cut, N_rad, R_inner, R_out, DPhi, Phi_0, op_rad,
    ...
    for fr_ind in op_fr:
        for rad_ind in op_rad:
            ...
            Phi_end = [ (Phi_beg + i*DPhi[rad_ind]) % 360. for i in range(1, N_az[rad_ind]+1) ]
            if track_mode:
                min_refang = DPhi[rad_ind]/2.
            ...
            for az_ind in op_az[rad_ind]:
                ...
                if az_ind == 0:
                    ...
                    Phi1 = Phi_end[az_ind-1]
                if Phi1 == Phi2: # 360-degree azimuthal region
                    mask_logic = np.vstack((np.less_equal(rad_vec, R2),\
                                            np.greater(rad_vec, R1)))
                elif Phi1 < Phi2:
                    mask_logic = np.vstack((np.less_equal(rad_vec, R2),\
                ...
                if fr_ind == op_fr[0] and diagnos_stride > 0:
                    print 'Search zone size for rad ind %d, az_ind %d (phi range [%.1f, %.1f]) is %d pixels' %\
                          (rad_ind, az_ind, Phi1, Phi2, zonemask_1d.shape[0])
...
def do_mp_klip_subtraction(N_proc, data_cube, config_dict, result_dict, result_dir, diagnos_stride=50, store_psf=False,
    ...
    sys.stdout.write("Took %dm%02ds to KLIP-subtract %d frames (%0.2f s per frame).\n" %\
...
#    return total_sumofsq_cost
    return total_sumofsq_cost/len(op_fr)

def mp_eval_twoobs_adiklip_srcmodel(p, N_proc, obs1_op_fr, obs2_op_fr, rad_ind, az_ind, obs2to1_flux_scale_fac,
                                    obs1_weight, obs2_weight, obs1_mode_cut, obs2_mode_cut,
                                    obs1_adiklip_config, obs2_adiklip_config, 
                                    obs1_adiklip_data, obs2_adiklip_data, obs1_srcmodel, obs2_srcmodel):
    if obs1_op_fr == None:
        obs1_op_fr = obs1_adiklip_config['op_fr']
    if obs2_op_fr == None:
        obs2_op_fr = obs2_adiklip_config['op_fr']
    if rad_ind == None:
        rad_ind = 0
    if az_ind == None:
        az_ind = 0
    if obs1_mode_cut == None:
        obs1_mode_cut = obs1_adiklip_config['mode_cut'][rad_ind]
    else:
        assert obs1_mode_cut >= 0 and obs1_mode_cut <= obs1_adiklip_config['mode_cut'][rad_ind]
    if obs2_mode_cut == None:
        obs2_mode_cut = obs2_adiklip_config['mode_cut'][rad_ind]
    else:
        assert obs2_mode_cut >= 0 and obs2_mode_cut <= obs2_adiklip_config['mode_cut'][rad_ind]

    obs1_p = p.copy()
    obs2_p = np.array([p[0]*obs2to1_flux_scale_fac, p[1], p[2]])
    obs1_cost = mp_eval_adiklip_srcmodel(p = obs1_p, N_proc = N_proc, op_fr = obs1_op_fr, rad_ind = None, az_ind = None, mode_cut = obs1_mode_cut,
                                         adiklip_config = obs1_adiklip_config, adiklip_data = obs1_adiklip_data, srcmodel = obs1_srcmodel)
    obs2_cost = mp_eval_adiklip_srcmodel(p = obs2_p, N_proc = N_proc, op_fr = obs2_op_fr, rad_ind = None, az_ind = None, mode_cut = obs2_mode_cut,
                                         adiklip_config = obs2_adiklip_config, adiklip_data = obs2_adiklip_data, srcmodel = obs2_srcmodel)
    return (obs1_weight*obs1_cost + obs2_weight*obs2_cost)/(obs1_weight + obs2_weight)
...
#    N_az = [ int(np.ceil(360./DPhi[r])) for r in range(N_rad) ]
    ...
    #
    if fake_planets != None:
        fit_fluxes = fit_residual_fake_planets(fake_planets=fake_planets, psf_model_img=synth_psf_img, max_fit_rad=10*round(fwhm),
                                               planet_coadd_img=fakep_coadd_img, log_fobj=log_fobj)
        if len(fake_planets) == 1:
            coadd_rms, coadd_min_detect,\
            med_rms, med_min_detect,\
            psffit_min_detect = get_residual_fake_planet_stats(config_dict=klip_config, Phi_0=Phi_0, R_p=fake_planets[0][0],
                                                               PA_p=fake_planets[0][1], flux_p=fake_planets[0][2],
                                                               flux_p_fit=fit_fluxes[0], fwhm=int(round(fwhm)),
    ...
    if fake_planets != None:
        if len(fake_planets) == 1:
            return klip_config, klip_data, coadd_img, med_img, fakep_coadd_img, fakep_med_img,\
                   coadd_min_detect, med_min_detect, psffit_min_detect, fit_fluxes
        else:
            return klip_config, klip_data, coadd_img, med_img, fakep_coadd_img, fakep_med_img, fit_fluxes
    else:
...
def klipmod(ampguess, posguess_rho, posguess_theta, klipsub_archv_fname, klipsub_result_dir, klipmod_result_dir,
    ...
    final_srcmodel_res_img_fname = '%s/%s_final_srcmodel_res_img.fits' % (klipmod_result_dir, result_label)
    ...
    if do_MLE:
        ...
        p_sol, final_cost, info = fmin_l_bfgs_b(func = mp_eval_adiklip_srcmodel, x0 = p0,
                                                ...
                                                approx_grad = True, bounds = p_bounds, epsilon=1e-6, factr=1e4, maxfun=1e6, disp=2)
        end_time = time.time()
        exec_time = end_time - start_time
        ...
        klipmod_res_img = klipsub_res_img - final_srcmodel_img
        final_srcmodel_res_hdu = pyfits.PrimaryHDU(klipmod_res_img.astype(np.float32))
        final_srcmodel_res_hdu.writeto(final_srcmodel_res_img_fname, clobber=True)
        print "Wrote the final source model residual to %s" % final_srcmodel_res_img_fname

        return p_sol
    else:
        return p0

def twoobs_klipmod(ampguess, posguess_rho, posguess_theta, obs2to1_flux_scale_fac, obs1_weight, obs2_weight,
                   obs1_klipsub_archv_fname, obs2_klipsub_archv_fname, obs1_klipsub_result_dir, obs2_klipsub_result_dir,
                   obs1_klipmod_result_dir, obs2_klipmod_result_dir, obs1_template_img_fname, obs2_template_img_fname,
                   obs1_synthpsf_fname, obs2_synthpsf_fname, synthpsf_rolloff, result_label,
                   N_proc, obs1_mode_cut=None, obs2_mode_cut=None, do_MLE=True):

    klipsub_src_img_fname = '%s/%s_src_img.fits' % (obs1_klipmod_result_dir, result_label)
    guess_srcmodel_fname = '%s/%s_guess_srcmodel_img.fits' % (obs1_klipmod_result_dir, result_label)
    final_srcmodel_fname = '%s/%s_final_srcmodel_img.fits' % (obs1_klipmod_result_dir, result_label)
    final_srcmodel_res_img_fname = '%s/%s_final_srcmodel_res_img.fits' % (obs1_klipmod_result_dir, result_label)

    assert os.path.exists(obs1_klipsub_result_dir)
    assert os.path.exists(obs2_klipsub_result_dir)
    assert os.path.exists(obs1_klipmod_result_dir)
    assert os.path.exists(obs2_klipmod_result_dir)
    assert os.path.exists(obs1_klipsub_archv_fname)
    assert os.path.exists(obs2_klipsub_archv_fname)

    obs1_klipsub_res_img_hdulist = pyfits.open(obs1_template_img_fname, 'readonly')
    obs1_klipsub_res_img = obs1_klipsub_res_img_hdulist[0].data
    obs1_klipsub_res_img_hdulist.close()
    obs1_exclude_ind = np.isnan(obs1_klipsub_res_img)

    obs2_klipsub_res_img_hdulist = pyfits.open(obs2_template_img_fname, 'readonly')
    obs2_klipsub_res_img = obs2_klipsub_res_img_hdulist[0].data
    obs2_klipsub_res_img_hdulist.close()
    obs2_exclude_ind = np.isnan(obs2_klipsub_res_img)

    comb_klipsub_src_img = (obs1_weight*obs1_klipsub_res_img + obs2_weight*obs2_klipsub_res_img/obs2to1_flux_scale_fac)/(obs1_weight + obs2_weight)

    obs1_klipsub_archv = open(obs1_klipsub_archv_fname, 'rb')
    print 'Opened KLIP subtraction archive %s' % obs1_klipsub_archv_fname
    print 'Loading the configuration and data...'
    obs1_klip_config, obs1_klip_data = pickle.load(obs1_klipsub_archv)
    obs1_klipsub_archv.close()

    obs2_klipsub_archv = open(obs2_klipsub_archv_fname, 'rb')
    print 'Opened KLIP subtraction archive %s' % obs2_klipsub_archv_fname
    print 'Loading the configuration and data...'
    obs2_klip_config, obs2_klip_data = pickle.load(obs2_klipsub_archv)
    obs2_klipsub_archv.close()

    obs1_fr_shape = obs1_klip_config['fr_shape']
    obs1_parang_seq = obs1_klip_config['parang_seq']
    obs1_op_fr = obs1_klip_config['op_fr']
    obs1_op_rad = obs1_klip_config['op_rad']
    obs1_op_az = obs1_klip_config['op_az']
    obs1_ref_table = obs1_klip_config['ref_table']
    obs1_zonemask_table_1d = obs1_klip_config['zonemask_table_1d']
    obs1_zonemask_table_2d = obs1_klip_config['zonemask_table_2d']
    obs1_N_op_fr = len(obs1_op_fr)

    obs2_fr_shape = obs2_klip_config['fr_shape']
    obs2_parang_seq = obs2_klip_config['parang_seq']
    obs2_op_fr = obs2_klip_config['op_fr']
    obs2_op_rad = obs2_klip_config['op_rad']
    obs2_op_az = obs2_klip_config['op_az']
    obs2_ref_table = obs2_klip_config['ref_table']
    obs2_zonemask_table_1d = obs2_klip_config['zonemask_table_1d']
    obs2_zonemask_table_2d = obs2_klip_config['zonemask_table_2d']
    obs2_N_op_fr = len(obs2_op_fr)
    #
    # Load PSF model, crop, and apply "roll-off" window to keep the edges smooth.
    #
    obs1_synthpsf_hdulist = pyfits.open(obs1_synthpsf_fname)
    obs1_synthpsf_img = obs1_synthpsf_hdulist[0].data
    obs1_synthpsf_hdulist.close()
    obs2_synthpsf_hdulist = pyfits.open(obs2_synthpsf_fname)
    obs2_synthpsf_img = obs2_synthpsf_hdulist[0].data
    obs2_synthpsf_hdulist.close()
    synthpsf_cent_xy = ((obs1_synthpsf_img.shape[0] - 1.)/2., (obs1_synthpsf_img.shape[1] - 1.)/2.)
    rolloff_rad = synthpsf_rolloff
    cropmarg = 1
    obs1_crop_synthpsf_img = obs1_synthpsf_img[cropmarg:-cropmarg, cropmarg:-cropmarg].copy()
    obs2_crop_synthpsf_img = obs2_synthpsf_img[cropmarg:-cropmarg, cropmarg:-cropmarg].copy()
    crop_synthpsf_cent_xy = ((obs1_crop_synthpsf_img.shape[0] - 1.)/2., (obs1_crop_synthpsf_img.shape[1] - 1.)/2.)
    Y, X = np.indices(obs1_crop_synthpsf_img.shape)
    Rsqrd = (X - crop_synthpsf_cent_xy[0])**2 + (Y - crop_synthpsf_cent_xy[1])**2
    obs1_crop_synthpsf_img *= np.exp( -(Rsqrd / rolloff_rad**2)**2 )
    obs2_crop_synthpsf_img *= np.exp( -(Rsqrd / rolloff_rad**2)**2 )
    #print synthpsf_img.shape, crop_synthpsf_img.shape
    print 'Sum of synthetic obs1 PSF before / after rolloff and cropping: %0.3f, %0.3f' % (obs1_synthpsf_img.sum(), obs1_crop_synthpsf_img.sum())
    print 'Sum of synthetic obs2 PSF before / after rolloff and cropping: %0.3f, %0.3f' % (obs2_synthpsf_img.sum(), obs2_crop_synthpsf_img.sum())

    cent_xy = ((obs1_fr_shape[1] - 1)/2., (obs1_fr_shape[0] - 1)/2.)
    posguess_xy = ( cent_xy[0] + posguess_rho*np.cos(np.deg2rad(posguess_theta + 90)),\
                    cent_xy[1] + posguess_rho*np.sin(np.deg2rad(posguess_theta + 90)) )
    posguess_deltaxy = (posguess_xy[0] - cent_xy[0], posguess_xy[1] - cent_xy[1])
    ...
    start_time = time.time()
    guess_cost = mp_eval_twoobs_adiklip_srcmodel(p=p0, N_proc=N_proc, obs1_op_fr=None, obs2_op_fr=None, rad_ind=None, az_ind=None,
                                                 obs2to1_flux_scale_fac=obs2to1_flux_scale_fac,
                                                 obs1_weight=obs1_weight, obs2_weight=obs2_weight,
                                                 obs1_mode_cut = obs1_mode_cut, obs2_mode_cut = obs2_mode_cut,
                                                 obs1_adiklip_config = obs1_klip_config, obs2_adiklip_config = obs2_klip_config, 
                                                 obs1_adiklip_data = obs1_klip_data, obs2_adiklip_data = obs2_klip_data,
                                                 obs1_srcmodel = obs1_crop_synthpsf_img, obs2_srcmodel = obs2_crop_synthpsf_img)
    end_time = time.time()
    exec_time = end_time - start_time
    print "Guess param cost func evaluation = %.1f. Took %dm%02ds to evaluate the joint two-observation KLIP source model cost" %\
          (guess_cost, int(exec_time/60.), exec_time - 60*int(exec_time/60.))
    ...
    if do_MLE:
        ...
        #                                        args = (op_fr, klip_config, klip_data, synthpsf_img),
        #                                        approx_grad = True, bounds = p_bounds, factr=1e8, maxfun=100, disp=2)
        p_sol, final_cost, info = fmin_l_bfgs_b(func = mp_eval_twoobs_adiklip_srcmodel, x0 = p0,
                                                args = (N_proc, None, None, None, None, obs2to1_flux_scale_fac,
                                                        obs1_weight, obs2_weight,
                                                        obs1_mode_cut, obs2_mode_cut,
                                                        obs1_klip_config, obs2_klip_config,
                                                        obs1_klip_data, obs2_klip_data,
                                                        obs1_crop_synthpsf_img, obs2_crop_synthpsf_img),
                                                approx_grad = True, bounds = p_bounds, epsilon=1e-6, factr=1e4, maxfun=1e6, disp=2)
        end_time = time.time()
        exec_time = end_time - start_time

        print "p_sol:", p_sol
        print "Took %dm%02ds to optimize the joint two-observation KLIP source model" %\
              (int(exec_time/60.), exec_time - 60*int(exec_time/60.))
        ...
        mle_sol_amp = p_sol[0]
        mle_sol_pos_xy = ( cent_xy[0] + p_sol[1], cent_xy[1] + p_sol[2] )
        mle_planet_psf = (obs1_weight*mle_sol_amp*obs1_crop_synthpsf_img + obs2_weight*mle_sol_amp*obs2_crop_synthpsf_img)/(obs1_weight + obs2_weight)
        final_srcmodel_img = superpose_srcmodel(data_img = np.zeros(obs1_fr_shape), srcmodel_img = mle_planet_psf,
                                                srcmodel_destxy = mle_sol_pos_xy, srcmodel_centxy = crop_synthpsf_cent_xy)
        final_srcmodel_img[obs1_exclude_ind] = np.nan
        final_srcmodel_img[obs2_exclude_ind] = np.nan
        final_srcmodel_hdu = pyfits.PrimaryHDU(final_srcmodel_img.astype(np.float32))
        final_srcmodel_hdu.writeto(final_srcmodel_fname, clobber=True)
        print "Wrote the final source model to %s" % final_srcmodel_fname
        klipmod_res_img = comb_klipsub_src_img - final_srcmodel_img
        klipsub_src_hdu = pyfits.PrimaryHDU(comb_klipsub_src_img.astype(np.float32))
        klipsub_src_hdu.writeto(klipsub_src_img_fname, clobber=True)
        print "Wrote the combined source image to %s" % klipsub_src_img_fname
        final_srcmodel_res_hdu = pyfits.PrimaryHDU(klipmod_res_img.astype(np.float32))
        final_srcmodel_res_hdu.writeto(final_srcmodel_res_img_fname, clobber=True)
        print "Wrote the final source model residual to %s" % final_srcmodel_res_img_fname
